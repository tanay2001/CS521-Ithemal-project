...
class Train():
    ...
    """
    def print_final(self,f,x,y,code_id):
        ...
        if x.shape != ():
            ...
            for i in range(size):
                # f.write('%f,%f ' % (x[i],y[i]))
                f.write('%s,%f,%f ' % (code_id[i], x[i], y[i]))
            f.write('\n')
        else:
            # print(f"{code_id},{x:.6f},{y:.6f}\n")
            f.write(f"{code_id},{x:.6f},{y:.6f}\n")
    ...
    def validate(self, resultfile, loadfile=None):
        ...
        for j, item in enumerate(tqdm(self.data.test)):
            ...
            # write to filw
            self.print_final(f, output, target, code_id)
        ...
        print("val", average_loss, self.correct, len(self.data.test))
        f.close()
