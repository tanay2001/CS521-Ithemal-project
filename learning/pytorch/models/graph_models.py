...
class RNN(AbstractGraphModule):
    ...
    def __init__(self, params):
        ...
        self.params = params
        # print the model type and details of architecture (layers, nodes, etc.)
        print('RNN model type: {}'.format(params.rnn_type))
        print('RNN model hierarchy type: {}'.format(params.hierarchy_type))
        print('RNN model connect tokens: {}'.format(params.connect_tokens))
        print('RNN model skip connections: {}'.format(params.skip_connections))
        print('RNN model learn init: {}'.format(params.learn_init))
        print('RNN model embedding size: {}'.format(params.embedding_size))
        print('RNN model hidden size: {}'.format(params.hidden_size))
        print('RNN model num classes: {}'.format(params.num_classes))
         # Print model summary
        ...
        print(f"Token RNN: {self.token_rnn}")
        print(f"Instr RNN: {self.instr_rnn}")
    ...
    def pred_of_instr_chain(self, instr_chain):
        ...
        try:
            ...
            # print("try", final_state_packed[0].shape)
        except:
            # define 0 tensor of shape torch.Size([1, 1, 256])
            print("except", instr_chain.shape)
            final_state_packed = (torch.zeros(1, 1, self.hidden_size), 
    ...
    def forward(self, item):
        ...
        tensors = [token_output_map[instr][-1] for instr in item.block.instrs
                if instr in token_output_map and len(token_output_map[instr]) > 0]

        if tensors:
            instr_chain = torch.stack(tensors)
        else:
            instr_chain = torch.empty(0)  # or handle appropriately
